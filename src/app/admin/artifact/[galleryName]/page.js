...
import { fetchArtifactsByGallery, updateArtifact, deleteArtifact, createArtifact } from '@/lib/api/artifact';
...
export default function GalleryArtifactsPage() {
    ...
    const confirmDelete = async () => {
        ...
        try {
            await deleteArtifact(confirmDeleteId, token);
            setArtifacts((prev) => prev.filter((a) => a._id !== confirmDeleteId));
        } catch (err) {
            setError(err.message);
        } finally {
    ...
    return (
        <div className="p-6 min-h-screen bg-white dark:bg-gray-900 relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 justify-items-center">
                {artifacts.map((artifact) => (
                    <div
                        key={artifact._id}
                        className="relative bg-white dark:bg-gray-800 shadow rounded-xl w-full max-w-sm group overflow-hidden"
                    >
                        <div className="absolute top-2 right-2 z-10 flex gap-2">
                            <button onClick={() => handleEdit(artifact)} className="text-blue-500 hover:text-blue-700">
                                <Pencil size={20} />
                            </button>
                            <button onClick={() => setConfirmDeleteId(artifact._id)} className="text-red-500 hover:text-red-700">
                                <Trash size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-4">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{artifact.artist}</p>
                ...
                <button
                    ...
                    className="rounded-xl p-6 shadow-lg bg-blue-100 dark:bg-gray-700 text-4xl font-bold text-blue-600 dark:text-white hover:bg-blue-200 dark:hover:bg-gray-600 w-full max-w-sm h-[400px]"
                >
                    +
                </button>
            </div>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        ...
                        <form onSubmit={handleUpdate} className="space-y-4">
                            <input name="name" value={formData.name} onChange={handleFormChange} placeholder="Name" className="w-full px-3 py-2 border rounded dark:bg-gray-700 dark:text-white" required />
